...
CASE WHEN b.facid = 0 THEN 'Tennis Court 1'
     WHEN b. facid = 1 THEN 'Tennis Court 2' END AS court
FROM Bookings as b
...
WHERE 
	(CASE WHEN b.memid = 0 THEN f.guestcost * b.slots
	ELSE f.membercost * b.slots END > 30) AND (b.starttime LIKE "2012-09-14%")
ORDER BY cost DESC

/* Q9: This time, produce the same result as in Q8, but using a subquery. */

SELECT sub.member, sub.facility, sub.cost
FROM 
	(SELECT CONCAT(m.firstname,' ', m.surname) AS member,
       		f.name AS facility,
       	CASE WHEN b.memid = 0 THEN f.guestcost * b.slots
